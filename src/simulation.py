...
from src.journal import Trade, TradePosition, EntryTime, RR, Outcome, MultiTimeframeAnalysis, Confidence, RiskManagement, Account
import random
import scipy.stats
...
def generate_simulated_trades(config: SimConfig) -> List[Trade]:
            ...
            for hour in range(24):
                ...
                if in_session and daily_trades < config.max_trades_per_day:
                    if random.random() < config.trade_probability_per_hour:
                        # Create trade
                        trade = Trade(uid=f"sim_{trade_count}")
...
def analyze_simulation(trades: List[Trade]) -> Dict:
...
def plot_simulation_results(trades: List[Trade], title: str = "Simulation Results"):
...
def run_simulation(config: Optional[SimConfig] = None) -> Tuple[List[Trade], Dict]:
    """
